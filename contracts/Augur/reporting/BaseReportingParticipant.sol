...
contract BaseReportingParticipant is IReportingParticipant {
    ...
    function liquidateLosing() public {
        ...
        require(
            market.getWinningPayoutDistributionHash() !=
                ...
                market.getWinningPayoutDistributionHash() != bytes32(0)
        );
        IReputationToken _reputationToken = market.getReputationToken();
        require(
            _reputationToken.transfer(
                address(market),
                _reputationToken.balanceOf(address(this))
            )
        );
    }
    ...
    function fork() public {
        ...
        IUniverse _newUniverse =
            market.getUniverse().createChildUniverse(payoutNumerators);
        IReputationToken _newReputationToken =
            _newUniverse.getReputationToken();
        uint256 _balance = reputationToken.balanceOf(address(this));
        reputationToken.migrateOutByPayout(
            _newUniverse.getPayoutNumerators(),
            _balance
        );
    ...
    function isDisavowed() public view returns (bool) {
        return
            market == IMarket(0) ||
            !market.isContainerForReportingParticipant(this);
    }
     ...
     */
    function getPayoutNumerator(uint256 _outcome)
        public
        view
        returns (uint256)
    {
