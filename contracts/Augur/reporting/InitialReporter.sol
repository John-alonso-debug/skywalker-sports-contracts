    ...
    IInitialReporter
{
    ...
    function redeem(address) public returns (bool) {
        ...
        if (!_isDisavowed) {
            augur.logInitialReporterRedeemed(
                market.getUniverse(),
                owner,
                address(market),
                size,
    ...
    function report(
        ...
        require(
            reportTimestamp == 0,
            'InitialReporter.report: Report has already been placed'
        );
        uint256 _timestamp = augur.getTimestamp();
        bool _isDesignatedReporter = _reporter == getDesignatedReporter();
        bool _designatedReportingExpired =
            _timestamp > market.getDesignatedReportingEndTime();
        require(
            _designatedReportingExpired || _isDesignatedReporter,
            'InitialReporter.report: Reporting time not started'
        );
    ...
    function returnRepFromDisavow() public {
        ...
        require(
            reputationToken.transfer(
                owner,
                reputationToken.balanceOf(address(this))
            )
        );
        reportTimestamp = 0;
    }
    ...
    function initialReporterWasCorrect() public view returns (bool) {
        return
            payoutDistributionHash != bytes32(0) &&
            payoutDistributionHash == market.getWinningPayoutDistributionHash();
    }

    function onTransferOwnership(address _owner, address _newOwner) internal {
        augur.logInitialReporterTransferred(
            market.getUniverse(),
            market,
            _owner,
            _newOwner
